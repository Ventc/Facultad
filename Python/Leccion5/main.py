...
def listarTerminos(**terminos): # Lo mas utilizando es **kwargs para recibir los argumentos
    for llave, valor in terminos.items():
        print(f"{llave} : {valor}")

listarTerminos() # No recibe nada, nada se va a mostrar
listarTerminos(IDE = "Integrated Development Enviroment", PK="Primary Key")
listarTerminos(Nombre="Ludmila Rossi")

def desplegarNombres(nombres):
    for nombre in nombres:
        print(nombre)
nombres2 = ["Ludmila", "Julieta", "Kevin"]
desplegarNombres(nombres2)
desplegarNombres("Ludmila")
...
def factorial(numero):
    if numero == 1: # Caso base
        ...
        return numero * factorial(numero-1) # Caso recursiva
